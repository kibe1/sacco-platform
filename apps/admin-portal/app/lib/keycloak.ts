...
export const clientId = process.env.NEXT_PUBLIC_KEYCLOAK_CLIENT_ID ?? "admin-portal";
export const appBaseUrl = process.env.NEXT_PUBLIC_APP_BASE_URL ?? "http://localhost:3100";

export function createKeycloakLoginUrl() {
  return createKeycloakAuthorizeUrl("admin-portal-local");
}
